...
namespace Spanjaan\BlogHub\Classes;
    ...
    */
    public function __construct(Post $model)
    {
        ...
        if ($ctrl && ($layout = $ctrl->getLayout()) !== null) {
            ...
            $model->spanjaan_bloghub_tags->each(fn ($tag) => $tag->setUrl($props['archiveTag'], $ctrl));
        }
    }
        ...
        public function getDetailReadTime($string = true)
        {
            ...
            if (!$string) {
                ...
                if ($minutes === 0) {
                    return trans('spanjaan.bloghub::lang.model.post.read_time_sec', [
                        'sec' => $seconds
                    ]);
                } else {
                    return trans('spanjaan.bloghub::lang.model.post.read_time', [
                        'min' => $minutes,
                        'sec' => $seconds
                    ]);
                }
        ...
        {
            return $this->model->spanjaan_bloghub_comments;
        }
        ...
        {
            return $this->model->spanjaan_bloghub_comments->count();
        }
        ...
        public function getTags()
        {
            if (empty($this->tagCollection)) {
                $this->tagCollection = $this->model->spanjaan_bloghub_tags;
        ...
        public function getPromotedTags()
        {
            if (empty($this->promotedTagCollection)) {
                $this->promotedTagCollection = $this->model->spanjaan_bloghub_tags->where('promote', '1');
        ...
        {
            if (!empty($this->model->spanjaan_bloghub_views)) {
                return intval($this->model->spanjaan_bloghub_views);
            } else {
                return 0;
            }
        }
        ...
        {
            if (!empty($this->model->spanjaan_bloghub_unique_views)) {
                return intval($this->model->spanjaan_bloghub_unique_views);
            } else {
                return 0;
            }
        }
        ...
        {
            $tags = $this->model->spanjaan_bloghub_tags->map(fn ($item) => $item->id)->all();
            $categories = $this->model->categories->map(fn ($item) => $item->id)->all();
            ...
            // Query
            $query = Post::with(['categories', 'featured_images', 'spanjaan_bloghub_tags'])
                ->whereHas('categories', function ($query) use ($categories) {
                    return $query->whereIn('winter_blog_categories.id', $categories);
                })
                ->whereHas('spanjaan_bloghub_tags', function ($query) use ($tags) {
                    return $query->whereIn('spanjaan_bloghub_tags.id', $tags);
                })
                ->limit($limit);
        ...
        public function getRandom($limit = 5, $exclude = [])
        {
            ...
            // Query
            $query = Post::with(['categories', 'featured_images', 'spanjaan_bloghub_tags'])->limit($limit);
