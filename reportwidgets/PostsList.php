...
class PostsList extends ReportWidgetBase
{
    ...
    public function defineProperties()
    {
        return [
            ...
            'defaultOrder' => [
                'title'         => 'spanjaan.bloghub::lang.components.post.default_order',
                'description'   => 'spanjaan.bloghub::lang.components.post.default_order_comment',
                'type'          => 'dropdown',
                'default'       => 'blog/post',
            ]
        ];
    ...
    {
        return [
            'by_published' => Lang::get('spanjaan.bloghub::lang.components.post.by_published'),
            'by_views' => Lang::get('spanjaan.bloghub::lang.components.post.by_views'),
            'by_visitors' => Lang::get('spanjaan.bloghub::lang.components.post.by_visitors')
        ];
    }
    ...
    protected function getPosts($order)
    {
        ...
        if ($order === 'by_published') {
            ...
            $posts = $query
                ->where('published', '1')
                ->orderBy('spanjaan_bloghub_unique_views', 'DESC')
                ->orderBy('published_at', 'DESC')
                ->get();
        } else {
            $posts = $query->get();
