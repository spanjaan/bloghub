...
namespace Spanjaan\BlogHub\ReportWidgets;
...
class CommentsList extends ReportWidgetBase
{
    ...
    public function defineProperties()
    {
        return [
            ...
            'defaultTab' => [
                'title'         => 'spanjaan.bloghub::lang.components.comments_list.default_tab',
                'description'   => 'spanjaan.bloghub::lang.components.comments_list.default_tab_comment',
                'type'          => 'dropdown',
                'options'       => [
                    'pending'   => Lang::get('spanjaan.bloghub::lang.model.comments.statusPending'),
                    'accepted'  => Lang::get('spanjaan.bloghub::lang.model.comments.statusAccepted'),
                    'rejected'  => Lang::get('spanjaan.bloghub::lang.model.comments.statusRejected'),
                    'spam'      => Lang::get('spanjaan.bloghub::lang.model.comments.statusSpam')
                ]
            ],
        ];
    }
    ...
    {
        $this->addCss('/plugins/spanjaan/bloghub/assets/css/widget-commentslist.css');
        $this->addCss('/plugins/spanjaan/bloghub/assets/css/widget-octoberv2.css');
    }
    ...
    public function onChangeStatus()
    {
        ...
        if(!(BackendAuth::check() && BackendAuth::getUser()->hasPermission('spanjaan.bloghub.comments.moderate_comments'))) {
            throw new AjaxException(Lang::get('spanjaan.bloghub::lang.frontend.errors.moderate_permission'));
        }

        // Validate Status
        if (!in_array($status, ['approve', 'reject', 'spam'])) {
            throw new AjaxException(Lang::get('spanjaan.bloghub::lang.frontend.errors.invalid_status'));
        }

        // Validate Comment ID
        if (empty($comment = Comment::where('id', $comment_id)->first())) {
            throw new AjaxException(Lang::get('spanjaan.bloghub::lang.frontend.errors.unknown_status'));
        }
        ...
        // Return Response
        return [
            'status' => Lang::get('spanjaan.bloghub::lang.frontend.success.update_status'),
            'counts' => [
                'pending' => Comment::where('status', 'pending')->count(),
                'approved' => Comment::where('status', 'approved')->count(),
                'rejected' => Comment::where('status', 'rejected')->count(),
